...
为推免（推荐免试研究生）面试准备的**纯前端**学习工作台。数据全部保存在浏览器 localStorage 中，无需后端。
...
| 📚 面试题库 A | 分类题库（基础/专业/英文/翻译…），支持收藏 ⭐、🔊朗读、搜索、批量导入、随机抽题、全真面试模式 |
| 🔥 重点复习 | 全工作台「不会」内容自动归档（题库/真题/词汇/翻译/单词），仅手动移出 |
| 🗂 真题分区 | 用户上传 6 份文档的真题总库，可编辑/添加/删除/标记 |
| 🎤 自我介绍 B | 三套文稿 + 背诵秒表 + 语音输入纠错 + 字数/时长估算 + 打卡 |
| 🔤 词汇·文献 C | 150 词专业词汇闪卡（含弱项模式）、文献翻译练习（点词查词/朗读/加入不会库）、每日顶刊推荐 |
